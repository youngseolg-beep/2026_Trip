                ...
                <tr key={exp.id} className="text-sm">
                  <td className="p-3">
                    ...
                    <p className="text-[9px] text-slate-400 font-medium">
                      {exp.date} · {exp.country} · <span className="text-blue-500 font-bold">{exp.category}</span>
                    </p>
                  </td>
      ...
      {isAdding ? (
        <form onSubmit={handleSubmit} className="p-4 bg-white rounded-2xl border-2 border-[#1d3557] space-y-3 shadow-xl mx-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* 구분(카테고리) 선택 - 추가됨 */}
            <select className="p-2 text-sm border rounded-lg bg-white font-bold text-blue-600" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
          ...
          <div className="grid grid-cols-1 gap-2">
            {/* 결제 수단 선택 */}
            <select className="p-2 text-sm border rounded-lg bg-white font-bold text-[#e63946]" value={formData.payment_method} onChange={e => setFormData({...formData, payment_method: e.target.value})}>
          ...
          <input type="text" placeholder="결제 내역 (장소 등)" className="w-full p-2 text-sm border rounded-lg bg-white" value={formData.item} onChange={e => setFormData({...formData, item: e.target.value})} />
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select className="p-2 text-sm border rounded-lg bg-white font-bold" value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value})}>
          ...
          <div className="flex gap-2">
            <button type="submit" className="flex-1 bg-[#1d3557] text-white p-2 rounded-lg text-sm font-bold shadow-sm">저장하기</button>
