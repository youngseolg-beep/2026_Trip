...
export function Expenses() {
  ...
  return (
    <div className="space-y-4 pb-24 px-1">
            ...
            {expenses.length === 0 ? (
              ...
              expenses.map((exp) => (
                <tr key={exp.id} className="text-sm">
                  ...
                  <td className="p-3 text-right">
                    <button onClick={() => deleteExpense(exp.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))
            )}
      ...
      {/* 지출 추가 폼 (캘린더 포함) */}
      {isAdding ? (
        <form onSubmit={handleSubmit} className="p-4 bg-white rounded-2xl border-2 border-[#1d3557] space-y-3 shadow-xl">
          <div className="flex items-center gap-2 mb-2">
            <Calendar size={16} className="text-[#1d3557]" />
            <span className="text-xs font-bold text-[#1d3557]">날짜 선택</span>
            <input 
              type="date" 
              className="flex-1 p-2 text-xs border rounded-lg bg-slate-50 outline-none" 
              value={formData.date} 
              onChange={e => setFormData({...formData, date: e.target.value})} 
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <select className="p-2 text-xs border rounded-lg" value={formData.country} onChange={e => setFormData({...formData, country: e.target.value})}>
              <option value="그리스">그리스</option>
              <option value="이스탄불">이스탄불</option>
              <option value="공통">공통</option>
            </select>
            <select className="p-2 text-xs border rounded-lg" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
          ...
          <input type="text" placeholder="결제 내역" className="w-full p-2 text-xs border rounded-lg" value={formData.item} onChange={e => setFormData({...formData, item: e.target.value})} />
          <div className="grid grid-cols-2 gap-2">
            ...
            <select className="p-2 text-xs border rounded-lg" value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value})}>
              <option value="유로">유로(€)</option><option value="원">원(₩)</option><option value="리라">리라(₺)</option>
            </select>
          </div>
          <div className="flex gap-2">
            <button type="submit" className="flex-1 bg-[#1d3557] text-white p-2 rounded-lg text-xs font-bold">저장</button>
            <button type="button" onClick={() => setIsAdding(false)} className="flex-1 bg-slate-100 p-2 rounded-lg text-xs">취소</button>
          </div>
        </form>
      ) : (
        <button onClick={() => setIsAdding(true)} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
          <Plus size={16} /> <span className="text-xs font-bold">지출 내역 추가</span>
        </button>
      )}
    </div>
  );
